...
object AppPrefs {
    ...
    private const val DEFAULT_WEBHOOK = "https://script.google.com/macros/s/AKfycbwozcu5hbK4QYjDBZqfXlnfNPdf9xFZ3lmKwX_8vCn_-W6tcftO9ePpqs3uW5tmlDkH/exec"
    private const val DEFAULT_RELAY_TOKEN = "a5a7pKzeZnUnxC5P-UlMYJOLbNZfbjXR4ppviwgU59s"

    private fun prefs(context: Context) = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)

    fun webhook(context: Context): String {
        val stored = prefs(context).getString(WEBHOOK, null).orEmpty().trim()
        return if (stored.startsWith("https://script.google.com/macros/s/") && stored.endsWith("/exec")) stored else DEFAULT_WEBHOOK
    }
